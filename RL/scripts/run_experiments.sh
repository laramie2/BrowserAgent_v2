...
#   TRAIN_ALGO=dapo bash RL/scripts/run_experiments.sh
...
# 强制全部重跑
#   FORCE_RERUN=1 bash RL/scripts/run_experiments.sh

# 跑指定的几组完整配置（设置后会跳过网格搜索）
#   EXPERIMENT_CONFIGS="n6_b64_lr3e-7_t0.8_kl0.01_warmup10 n6_b64_lr5e-7_t0.7_e4_kl0.005_warmup20" bash RL/scripts/run_experiments.sh
...
# 跳过失败实验
#   SKIP_FAILED=1 bash RL/scripts/run_experiments.sh

# 失败就停止
#   CONTINUE_ON_FAIL=0 bash RL/scripts/run_experiments.sh
...
if [ -f "$TRAIN_CONFIG" ]; then
  eval "$(python3 "$SCRIPT_DIR/config_to_env.py" auto "$TRAIN_CONFIG")"
fi
...
case "$TRAIN_ALGO" in
  mt_grpo|grpo|ppo|default)
    TRAIN_PRESET_EFFECTIVE="${TRAIN_PRESET:-mt_grpo}"
    ARTIFACT_SUFFIX_DEFAULT="${ARTIFACT_SUFFIX:-mt_grpo}"
    ;;
  dapo)
    TRAIN_PRESET_EFFECTIVE="${TRAIN_PRESET:-dapo}"
    ARTIFACT_SUFFIX_DEFAULT="${ARTIFACT_SUFFIX:-dapo}"
    ;;
  ppo_opd|opd)
    TRAIN_PRESET_EFFECTIVE="${TRAIN_PRESET:-ppo_opd}"
    ARTIFACT_SUFFIX_DEFAULT="${ARTIFACT_SUFFIX:-ppo_opd}"
    ;;
  *)
    echo "[CONFIG ERROR] unsupported TRAIN_ALGO='$TRAIN_ALGO'. Use mt_grpo, dapo, or ppo_opd." >&2
    exit 1
    ;;
esac

export TRAIN_PRESET="$TRAIN_PRESET_EFFECTIVE"

if [ -f "$TRAIN_CONFIG" ]; then
  eval "$(python3 "$SCRIPT_DIR/config_to_env.py" train "$TRAIN_CONFIG" "$TRAIN_PRESET")"
fi

ARTIFACT_SUFFIX_EFFECTIVE="${ARTIFACT_SUFFIX:-$ARTIFACT_SUFFIX_DEFAULT}"

case "$TRAIN_ALGO" in
  mt_grpo|grpo|ppo|default)
    TRAIN_SCRIPT="${TRAIN_SCRIPT_OVERRIDE:-$SCRIPT_DIR/train.sh}"
    LOG_ROOT="${LOG_ROOT_OVERRIDE:-$RL_DIR/logs/$ARTIFACT_SUFFIX_EFFECTIVE}"
    BASE_RAY_TMP_DEFAULT="/tmp/browseragent-ray"
    ;;
  dapo)
    TRAIN_SCRIPT="${TRAIN_SCRIPT_OVERRIDE:-$SCRIPT_DIR/train.sh}"
    LOG_ROOT="${LOG_ROOT_OVERRIDE:-$RL_DIR/logs/$ARTIFACT_SUFFIX_EFFECTIVE}"
...
REWARD_PROCESS_PENALTY_WEIGHT_VALUE="${REWARD_PROCESS_PENALTY_WEIGHT_OVERRIDE:-0.05}"
REWARD_RETRIEVAL_WEIGHT_VALUE="${REWARD_RETRIEVAL_WEIGHT_OVERRIDE:-0.20}"
REWARD_REFINEMENT_WEIGHT_VALUE="${REWARD_REFINEMENT_WEIGHT_OVERRIDE:-0.10}"
REWARD_QUERY_WEIGHT_VALUE="${REWARD_QUERY_WEIGHT_OVERRIDE:-0.05}"
REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE="${REWARD_ACTION_PENALTY_TURN_WEIGHT_OVERRIDE:-0.05}"
REWARD_RETRIEVAL_DECAY_VALUE="${REWARD_RETRIEVAL_DECAY_OVERRIDE:-0.05}"
TURN_ADVANTAGE_COEF_VALUE="${TURN_ADVANTAGE_COEF_OVERRIDE:-0.3}"
if [ "$REWARD_ENABLE_VALUE" != "True" ]; then
  ...
  REWARD_PROCESS_PENALTY_WEIGHT_VALUE="0.0"
  REWARD_RETRIEVAL_WEIGHT_VALUE="0.0"
  REWARD_REFINEMENT_WEIGHT_VALUE="0.0"
  REWARD_QUERY_WEIGHT_VALUE="0.0"
  REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE="0.0"
fi
...
PPO_MAX_TOKEN_LEN_PER_GPU_VALUE="${PPO_MAX_TOKEN_LEN_PER_GPU_OVERRIDE:-10240}"
TOP_P_VALUE="${TOP_P_OVERRIDE:-1.0}"
GPU_MEMORY_UTILIZATION_VALUE="${GPU_MEMORY_UTILIZATION_OVERRIDE:-}"
ENABLE_OBS_COMPRESSION_VALUE="${ENABLE_OBS_COMPRESSION_OVERRIDE:-True}"

OPD_ENABLE_VALUE="${OPD_ENABLE_OVERRIDE:-False}"
OPD_TEACHER_MODEL_VALUE="${OPD_TEACHER_MODEL_OVERRIDE:-}"
OPD_TEACHER_CUDA_VISIBLE_DEVICES_VALUE="${OPD_TEACHER_CUDA_VISIBLE_DEVICES:-}"
OPD_TEACHER_TP_SIZE_VALUE="${OPD_TEACHER_TP_SIZE_OVERRIDE:-1}"
OPD_TEACHER_DTYPE_VALUE="${OPD_TEACHER_DTYPE_OVERRIDE:-bfloat16}"
OPD_TEACHER_TOPK_VALUE="${OPD_TEACHER_TOPK_OVERRIDE:-20}"
OPD_TEACHER_BATCH_SIZE_VALUE="${OPD_TEACHER_BATCH_SIZE_OVERRIDE:-1}"
OPD_TEACHER_TIMEOUT_VALUE="${OPD_TEACHER_TIMEOUT_OVERRIDE:-600}"
OPD_REASON_COEF_VALUE="${OPD_REASON_COEF_OVERRIDE:-0.03}"
OPD_TEACHER_TEMPERATURE_VALUE="${OPD_TEACHER_TEMPERATURE_OVERRIDE:-1.0}"

SAVE_FREQ_VALUE="${SAVE_FREQ_OVERRIDE:--1}"
TEST_FREQ_VALUE="${TEST_FREQ_OVERRIDE:-5}"
SAVE_BEST_BY_METRIC_VALUE="${SAVE_BEST_BY_METRIC_OVERRIDE:-True}"
SAVE_BEST_METRIC_VALUE="${SAVE_BEST_METRIC_OVERRIDE:-val-core/all/reward/mean@1}"
SAVE_BEST_MODE_VALUE="${SAVE_BEST_MODE_OVERRIDE:-max}"
SAVE_BEST_MIN_DELTA_VALUE="${SAVE_BEST_MIN_DELTA_OVERRIDE:-0.0}"
SAVE_LAST_CHECKPOINT_VALUE="${SAVE_LAST_CHECKPOINT_OVERRIDE:-False}"
...
if [ -n "$RL_ALG_VALUE" ]; then
  ...
  case "$TRAIN_ALGO" in
    ...
    ppo_opd|opd) RL_ALG_EFFECTIVE="grpo" ;;
...
SUMMARY_HEADER="run_tag\tstatus\tbest_metric\texit_code\tn\tbatch\tlr\ttemp\tepoch\tkl_loss_coef\tlr_warmup_steps\tmax_turns\ttrain_preset\tartifact_suffix\trun_name_postfix\trl_alg\tuse_kl_loss\tdapo_filter_groups\tnorm_adv_std\tclip_low\tclip_high\tloss_agg_mode\tsampler_mode\treward_enable\treward_answer\treward_format\treward_process\treward_process_penalty\treward_retrieval\treward_refinement\treward_query\treward_action_penalty_turn\treward_retrieval_decay\tturn_advantage_coef\treward_action_correctness\treward_hallucinated_id\treward_tool_invalid\topd_enable\topd_reason_coef\topd_teacher_topk\tenable_obs_compression\ttop_p\tppo_max_token_len_per_gpu\tpass_extra_fields\tcompact_tool_info\tagent_workers\tmax_concurrent_traj\trollout_max_num_seqs\trollout_max_batched_tokens\tsave_best_metric\twandb_run\tinner_log\touter_log"

SUMMARY_HEADER_LINE="$(printf '%b' "$SUMMARY_HEADER")"
if [ ! -f "$SUMMARY_FILE" ]; then
  printf '%s
' "$SUMMARY_HEADER_LINE" > "$SUMMARY_FILE"
elif [ "$(head -n 1 "$SUMMARY_FILE")" != "$SUMMARY_HEADER_LINE" ]; then
...
cleanup_after_run() {
  ...
  pkill -u "$(id -u)" -f "verl_tool.servers.opd_teacher_server" || true
...
append_summary() {
  ...
  echo -e "${run_tag}	${status}	${best_metric}	${exit_code}	${n}	${batch}	${lr}	${temp}	${epoch}	${kl_loss_coef}	${lr_warmup_steps}	${max_turns}	${TRAIN_PRESET_EFFECTIVE}	${ARTIFACT_SUFFIX_EFFECTIVE}	${RUN_NAME_POSTFIX_VALUE}	${RL_ALG_EFFECTIVE}	${USE_KL_LOSS_EFFECTIVE}	${ENABLE_DAPO_FILTER_GROUPS_EFFECTIVE}	${NORM_ADV_BY_STD_IN_GRPO_VALUE}	${CLIP_RATIO_LOW_VALUE}	${CLIP_RATIO_HIGH_VALUE}	${LOSS_AGG_MODE_VALUE}	${SAMPLER_MODE}	${REWARD_ENABLE_VALUE}	${REWARD_ANSWER_WEIGHT_VALUE}	${REWARD_FORMAT_WEIGHT_VALUE}	${REWARD_ENABLE_PROCESS_VALUE}	${REWARD_PROCESS_PENALTY_WEIGHT_VALUE}	${REWARD_RETRIEVAL_WEIGHT_VALUE}	${REWARD_REFINEMENT_WEIGHT_VALUE}	${REWARD_QUERY_WEIGHT_VALUE}	${REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE}	${REWARD_RETRIEVAL_DECAY_VALUE}	${TURN_ADVANTAGE_COEF_VALUE}	${REWARD_ACTION_CORRECTNESS_WEIGHT_VALUE}	${REWARD_HALLUCINATED_ID_PENALTY_WEIGHT_VALUE}	${REWARD_TOOL_INVALID_PENALTY_WEIGHT_VALUE}	${OPD_ENABLE_VALUE}	${OPD_REASON_COEF_VALUE}	${OPD_TEACHER_TOPK_VALUE}	${ENABLE_OBS_COMPRESSION_VALUE}	${TOP_P_VALUE}	${PPO_MAX_TOKEN_LEN_PER_GPU_VALUE}	${PASS_EXTRA_FIELDS_TO_REWARD_VALUE}	${COMPACT_TOOL_INTERACT_INFO_VALUE}	${AGENT_LOOP_NUM_WORKERS_VALUE}	${MAX_CONCURRENT_TRAJECTORIES_VALUE}	${ROLLOUT_MAX_NUM_SEQS_VALUE}	${ROLLOUT_MAX_BATCHED_TOKENS_VALUE}	${SAVE_BEST_METRIC_VALUE}	${WANDB_RUN_NAME_CURRENT}	${inner_log}	${outer_log}" >> "$SUMMARY_FILE"
}
...
echo "TRAIN_PRESET=$TRAIN_PRESET_EFFECTIVE"
echo "ARTIFACT_SUFFIX=$ARTIFACT_SUFFIX_EFFECTIVE"
...
echo "REWARD enable=$REWARD_ENABLE_VALUE answer=$REWARD_ANSWER_WEIGHT_VALUE format=$REWARD_FORMAT_WEIGHT_VALUE process=$REWARD_ENABLE_PROCESS_VALUE process_penalty=$REWARD_PROCESS_PENALTY_WEIGHT_VALUE retrieval=$REWARD_RETRIEVAL_WEIGHT_VALUE refinement=$REWARD_REFINEMENT_WEIGHT_VALUE query=$REWARD_QUERY_WEIGHT_VALUE action_turn_penalty=$REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE retrieval_decay=$REWARD_RETRIEVAL_DECAY_VALUE turn_advantage=$TURN_ADVANTAGE_COEF_VALUE action=$REWARD_ACTION_CORRECTNESS_WEIGHT_VALUE hallucinated=$REWARD_HALLUCINATED_ID_PENALTY_WEIGHT_VALUE tool_invalid=$REWARD_TOOL_INVALID_PENALTY_WEIGHT_VALUE"
echo "OPD enable=$OPD_ENABLE_VALUE teacher_model=${OPD_TEACHER_MODEL_VALUE:-NA} teacher_gpus=${OPD_TEACHER_CUDA_VISIBLE_DEVICES_VALUE:-default} topk=$OPD_TEACHER_TOPK_VALUE coef=$OPD_REASON_COEF_VALUE obs_compression=$ENABLE_OBS_COMPRESSION_VALUE"
echo "CHECKPOINT save_freq=$SAVE_FREQ_VALUE test_freq=$TEST_FREQ_VALUE save_best=$SAVE_BEST_BY_METRIC_VALUE metric=$SAVE_BEST_METRIC_VALUE mode=$SAVE_BEST_MODE_VALUE save_last=$SAVE_LAST_CHECKPOINT_VALUE"
echo "CONCURRENCY agent_workers=$AGENT_LOOP_NUM_WORKERS_VALUE max_traj=$MAX_CONCURRENT_TRAJECTORIES_VALUE tool_requests=$TOOL_SERVER_MAX_CONCURRENT_REQUESTS_VALUE browser_actors=$TEXT_BROWSER_MAX_ACTIVE_ACTORS_VALUE rollout_seqs=$ROLLOUT_MAX_NUM_SEQS_VALUE ppo_max_token_len=$PPO_MAX_TOKEN_LEN_PER_GPU_VALUE top_p=$TOP_P_VALUE"
...
for config_row in "${CONFIG_ROWS[@]}"; do
  ...
  export TRAIN_PRESET="$TRAIN_PRESET_EFFECTIVE"
  export ARTIFACT_SUFFIX="$ARTIFACT_SUFFIX_EFFECTIVE"
  ...
  export REWARD_PROCESS_PENALTY_WEIGHT_OVERRIDE="$REWARD_PROCESS_PENALTY_WEIGHT_VALUE"
  export REWARD_RETRIEVAL_WEIGHT_OVERRIDE="$REWARD_RETRIEVAL_WEIGHT_VALUE"
  export REWARD_REFINEMENT_WEIGHT_OVERRIDE="$REWARD_REFINEMENT_WEIGHT_VALUE"
  export REWARD_QUERY_WEIGHT_OVERRIDE="$REWARD_QUERY_WEIGHT_VALUE"
  export REWARD_ACTION_PENALTY_TURN_WEIGHT_OVERRIDE="$REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE"
  export REWARD_RETRIEVAL_DECAY_OVERRIDE="$REWARD_RETRIEVAL_DECAY_VALUE"
  export TURN_ADVANTAGE_COEF_OVERRIDE="$TURN_ADVANTAGE_COEF_VALUE"
  ...
  export PPO_MAX_TOKEN_LEN_PER_GPU_OVERRIDE="$PPO_MAX_TOKEN_LEN_PER_GPU_VALUE"
  export TOP_P_OVERRIDE="$TOP_P_VALUE"
  export ENABLE_OBS_COMPRESSION_OVERRIDE="$ENABLE_OBS_COMPRESSION_VALUE"
  export OPD_ENABLE_OVERRIDE="$OPD_ENABLE_VALUE"
  export OPD_TEACHER_MODEL_OVERRIDE="$OPD_TEACHER_MODEL_VALUE"
  export OPD_TEACHER_CUDA_VISIBLE_DEVICES="$OPD_TEACHER_CUDA_VISIBLE_DEVICES_VALUE"
  export OPD_TEACHER_TP_SIZE_OVERRIDE="$OPD_TEACHER_TP_SIZE_VALUE"
  export OPD_TEACHER_DTYPE_OVERRIDE="$OPD_TEACHER_DTYPE_VALUE"
  export OPD_TEACHER_TOPK_OVERRIDE="$OPD_TEACHER_TOPK_VALUE"
  export OPD_TEACHER_BATCH_SIZE_OVERRIDE="$OPD_TEACHER_BATCH_SIZE_VALUE"
  export OPD_TEACHER_TIMEOUT_OVERRIDE="$OPD_TEACHER_TIMEOUT_VALUE"
  export OPD_REASON_COEF_OVERRIDE="$OPD_REASON_COEF_VALUE"
  export OPD_TEACHER_TEMPERATURE_OVERRIDE="$OPD_TEACHER_TEMPERATURE_VALUE"
  export SAVE_FREQ_OVERRIDE="$SAVE_FREQ_VALUE"
  export TEST_FREQ_OVERRIDE="$TEST_FREQ_VALUE"
  export SAVE_BEST_BY_METRIC_OVERRIDE="$SAVE_BEST_BY_METRIC_VALUE"
  export SAVE_BEST_METRIC_OVERRIDE="$SAVE_BEST_METRIC_VALUE"
  export SAVE_BEST_MODE_OVERRIDE="$SAVE_BEST_MODE_VALUE"
  export SAVE_BEST_MIN_DELTA_OVERRIDE="$SAVE_BEST_MIN_DELTA_VALUE"
  export SAVE_LAST_CHECKPOINT_OVERRIDE="$SAVE_LAST_CHECKPOINT_VALUE"
  ...
  echo "train_preset=$TRAIN_PRESET_EFFECTIVE artifact_suffix=$ARTIFACT_SUFFIX_EFFECTIVE run_name_postfix=$RUN_NAME_POSTFIX_VALUE" | tee -a "$OUTER_LOG"
  echo "n=$n batch=$batch lr=$lr temp=$temp top_p=$TOP_P_VALUE epoch=$epoch kl_loss_coef=$kl_loss_coef lr_warmup_steps=$lr_warmup_steps max_turns=${max_turns:-default}" | tee -a "$OUTER_LOG"
  ...
  echo "reward_enable=$REWARD_ENABLE_VALUE reward_answer=$REWARD_ANSWER_WEIGHT_VALUE reward_format=$REWARD_FORMAT_WEIGHT_VALUE reward_process=$REWARD_ENABLE_PROCESS_VALUE reward_process_penalty=$REWARD_PROCESS_PENALTY_WEIGHT_VALUE reward_retrieval=$REWARD_RETRIEVAL_WEIGHT_VALUE reward_refinement=$REWARD_REFINEMENT_WEIGHT_VALUE reward_query=$REWARD_QUERY_WEIGHT_VALUE reward_action_turn_penalty=$REWARD_ACTION_PENALTY_TURN_WEIGHT_VALUE reward_retrieval_decay=$REWARD_RETRIEVAL_DECAY_VALUE turn_advantage_coef=$TURN_ADVANTAGE_COEF_VALUE reward_action=$REWARD_ACTION_CORRECTNESS_WEIGHT_VALUE reward_hallucinated=$REWARD_HALLUCINATED_ID_PENALTY_WEIGHT_VALUE reward_tool_invalid=$REWARD_TOOL_INVALID_PENALTY_WEIGHT_VALUE" | tee -a "$OUTER_LOG"
  echo "opd_enable=$OPD_ENABLE_VALUE opd_teacher_model=${OPD_TEACHER_MODEL_VALUE:-NA} opd_teacher_gpus=${OPD_TEACHER_CUDA_VISIBLE_DEVICES_VALUE:-default} opd_topk=$OPD_TEACHER_TOPK_VALUE opd_coef=$OPD_REASON_COEF_VALUE obs_compression=$ENABLE_OBS_COMPRESSION_VALUE" | tee -a "$OUTER_LOG"
  echo "save_freq=$SAVE_FREQ_VALUE test_freq=$TEST_FREQ_VALUE save_best=$SAVE_BEST_BY_METRIC_VALUE save_best_metric=$SAVE_BEST_METRIC_VALUE save_best_mode=$SAVE_BEST_MODE_VALUE save_last=$SAVE_LAST_CHECKPOINT_VALUE" | tee -a "$OUTER_LOG"
  echo "agent_workers=$AGENT_LOOP_NUM_WORKERS_VALUE max_concurrent_trajectories=$MAX_CONCURRENT_TRAJECTORIES_VALUE tool_server_max_requests=$TOOL_SERVER_MAX_CONCURRENT_REQUESTS_VALUE compact_tool_interact_info=$COMPACT_TOOL_INTERACT_INFO_VALUE pass_extra_fields_to_reward=$PASS_EXTRA_FIELDS_TO_REWARD_VALUE rollout_max_num_seqs=$ROLLOUT_MAX_NUM_SEQS_VALUE ppo_max_token_len_per_gpu=$PPO_MAX_TOKEN_LEN_PER_GPU_VALUE" | tee -a "$OUTER_LOG"
