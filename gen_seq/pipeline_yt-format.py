...
class TextBrowserEnv:
    ...
    @staticmethod
    def _angle_args_to_square(action_text: str) -> str:
        return re.sub(r'<([^<>]+)>', r'[\1]', action_text)

    def _format_response_for_env(self, response: str) -> str:
        if not response:
            return response
        ...
        def format_code_block(match):
            content = self._angle_args_to_square(match.group(1).strip())
            return f"{ticks}{content}{ticks}"

        def format_action_tag(match):
            content = self._angle_args_to_square(match.group(1).strip())
    ...
    def step(self, trajectory_id: str, action: str, is_finish: bool) -> str:
        formatted_response = self._format_response_for_env(action)

        # print(formatted_response)

        data = {
            "trajectory_ids": [trajectory_id],
            "actions": [formatted_response],
            "finish": [is_finish],
...
"""
    ...
    def extract_stop_answer(self, action: str) -> str:
        match = re.search(r'^stop\s*(?:\[(.*?)\]|<(.*?)>)\s*$', action, re.IGNORECASE | re.DOTALL)
        if match:
            return (match.group(1) or match.group(2) or "").strip()
        return re.sub(r'^\s*stop\s*', '', action, flags=re.IGNORECASE).strip(" []<>")
    ...
    def run_single_episode(self, question: str, ground_truth: str, sample_idx: int, trial_idx: int = 1, max_steps: int = 30, model: str = "custom-llm") -> bool:
        ...
        for step in range(max_steps):
            ...
            if is_stop:
                final_answer = self.extract_stop_answer(action)
