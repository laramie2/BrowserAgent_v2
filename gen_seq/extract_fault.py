...
def analyze_trajectory_errors(file_path):
    ...
    # 打开四个用于写入的 jsonl 文件
    f_wrong_answer = open(os.path.join(output_dir, "1_wrong_answer_tasks.jsonl"), 'w', encoding='utf-8')
    f_loop = open(os.path.join(output_dir, "2_loop_tasks.jsonl"), 'w', encoding='utf-8')
    f_format_crash = open(os.path.join(output_dir, "3_format_crash_tasks.jsonl"), 'w', encoding='utf-8')
    f_max_steps = open(os.path.join(output_dir, "4_max_steps_tasks.jsonl"), 'w', encoding='utf-8')
    ...
    print(f"  -> 全局总动作数(Steps): {total_actions} | 其中格式错误动作数: {format_err_actions_count}")
    ...
    print(f"包含以下文件: \n - 1_wrong_answer_tasks.jsonl\n - 2_loop_tasks.jsonl\n - 3_format_crash_tasks.jsonl\n - 4_max_steps_tasks.jsonl")


if __name__ == "__main__":
    # 将此处的路径替换为您实际的 jsonl 文件路径
