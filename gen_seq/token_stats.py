#!/usr/bin/env python3
"""Count per-step prompt tokens for gen_seq inference results.

The two primary variants match gen_seq/pipeline.py:
...
DEFAULT_SYSTEM_PROMPT = PROJECT_ROOT / "gen_seq" / "system_prompt_with_history_info.txt"
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Count average and max per-step model input tokens for gen_seq results."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=PROJECT_ROOT / "gen_seq" / "results",
    ...
    parser.add_argument(
        ...
        help="System prompt used by gen_seq/pipeline.py.",
    )
    parser.add_argument(
        "--output_json",
        type=Path,
        default=PROJECT_ROOT / "gen_seq" / "results" / "token_usage_summary.json",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "pipeline rebuilds the one-image prompt from gen_seq/pipeline.py."
        ),
    )
    parser.add_argument(
        "--rerender_images",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    summary = {
        ...
        "notes": {
            "no_image_compression": "Raw text observation in the gen_seq/pipeline.py user prompt.",
            "image_compression": (
