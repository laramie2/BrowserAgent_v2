...
def ensure_answer_list(golden_answers):
    """Normalize scalar/array-like ground truths for matching."""
...
def substring_check(prediction, golden_answers):
    normalized_prediction = normalize_answer(prediction)
    for golden_answer in ensure_answer_list(golden_answers):
...
def extract_stop_answer(action):
    match = re.search(r"^\s*stop\s*(?:\[(.*?)\]|<(.*?)>)\s*$", str(action), re.IGNORECASE | re.DOTALL)
    if match:
        return (match.group(1) or match.group(2) or "").strip()
    return re.sub(r"^\s*stop\s*", "", str(action), flags=re.IGNORECASE).strip(" []<>")


def final_answer_from_record(data):
    for key in ("final_conclusion", "final_answer", "answer"):
        final_answer = data.get(key)
        if final_answer:
    ...
    for step in reversed(data.get("steps", []) or []):
        ...
        if "stop" in action.lower():
            return extract_stop_answer(action)
    return ""


def load_and_fill_success_fields(file_path):
    """Fill missing match fields, preserving values already produced by pipeline."""
    records = []
    output_lines = []
    stats = {
        "updated_success_em": 0,
        "updated_success_substring": 0,
        "removed_success_subtring": 0,
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            final_answer = final_answer_from_record(data)
            ground_truth = data.get("ground_truth")
            computed_em = bool(final_answer and ground_truth and em_check(final_answer, ground_truth))
            computed_substring = bool(final_answer and ground_truth and substring_check(final_answer, ground_truth))

            if "success_em" in data:
                success_em = bool(data["success_em"])
            else:
                success_em = computed_em
                data["success_em"] = success_em
                stats["updated_success_em"] += 1

            if "success_substring" in data:
                success_substring = bool(data["success_substring"])
            else:
                success_substring = computed_substring
                data["success_substring"] = success_substring
                stats["updated_success_substring"] += 1

            if "success_subtring" in data:
                data.pop("success_subtring", None)
                stats["removed_success_subtring"] += 1

            # Keep the current pipeline contract: success is the normalized EM result.
            if data.get("success") != success_em:
                data["success"] = success_em
                stats["normalized_success"] += 1

            records.append(data)
            output_lines.append(json.dumps(data, ensure_ascii=False) + "\n")

    if (
        stats["updated_success_em"]
        or stats["updated_success_substring"]
        or stats["removed_success_subtring"]
        or stats["normalized_success"]
    ):
        output_dir = os.path.dirname(os.path.abspath(file_path)) or "."
        fd, tmp_path = tempfile.mkstemp(prefix=".check_fault_", suffix=".jsonl", dir=output_dir, text=True)
        try:
...
def analyze_trajectory_errors(file_path):
    ...
    em_success_tasks = 0
    substring_success_tasks = 0
    field_patch_stats = None
    ...
    try:
        records, field_patch_stats = load_and_fill_success_fields(file_path)
        for data in records:
            task_success = bool(data.get("success", False))
            substring_success = bool(data.get("success_substring", False))

            total_tasks += 1
            em_success_tasks += int(task_success)
            substring_success_tasks += int(substring_success)
            ...
            for step in data.get('steps', []):
                ...
                if not match:
                    ...
                    extracted_action = (match.group(1) or match.group(2) or "").strip()
    ...
    print("="*80)
    em_success_rate = em_success_tasks / max(total_tasks, 1) * 100
    substring_success_rate = substring_success_tasks / max(total_tasks, 1) * 100
    print(f"总任务数: {total_tasks} | 失败任务数(按 success/EM 字段): {total_failed_tasks} (失败率: {total_failed_tasks/max(total_tasks,1)*100:.2f}%  |  EM成功率: {em_success_rate:.2f}%)")
    print(f"子串匹配成功率: {substring_success_tasks}/{total_tasks} ({substring_success_rate:.2f}%)")
    if field_patch_stats and field_patch_stats["rewritten"]:
        print(
            "已重算并回写字段: "
            f"success_em 更新 {field_patch_stats['updated_success_em']} 条, "
            f"success_substring 更新 {field_patch_stats['updated_success_substring']} 条, "
            f"移除 success_subtring {field_patch_stats['removed_success_subtring']} 条, "
            f"success 对齐EM {field_patch_stats['normalized_success']} 条"
        )
    print("-" * 80)
...
if __name__ == "__main__":
    TEST_FILE_DIR = "/home/nvidia/yutao/lzt/BrowserAgent_v2/gen_seq/results/Qwen2.5-VL-7B-Instruct-rl-2000-hotpot0.25-nq0.75" 

    NQ_FILE_PATH = f"{TEST_FILE_DIR}/nq_test_results.jsonl"
    TRIVIAQA_FILE_PATH = f"{TEST_FILE_DIR}/triviaqa_test_results.jsonl"
    POPQA_FILE_PATH = f"{TEST_FILE_DIR}/popqa_test_results.jsonl"
    HOTPOT_FILE_PATH = f"{TEST_FILE_DIR}/hotpot_test_results.jsonl"
    _2WIKI_FILE_PATH = f"{TEST_FILE_DIR}/2wiki_test_results.jsonl"
    MUSIQUE_FILE_PATH = f"{TEST_FILE_DIR}/musique_test_results.jsonl"
    BAMBOOGLE_FILE_PATH = f"{TEST_FILE_DIR}/bamboogle_test_results.jsonl"
    ...
    print("="*80)
    print("triviaqa测试结果")
    analyze_trajectory_errors(TRIVIAQA_FILE_PATH)
    print("="*80)
    print("popqa测试结果")
    analyze_trajectory_errors(POPQA_FILE_PATH)
    print("="*80)
    print("hotpot测试结果")
    ...
    print("="*80)
    print("2wiki测试结果")
    analyze_trajectory_errors(_2WIKI_FILE_PATH)
    print("="*80)
    print("musique测试结果")
    analyze_trajectory_errors(MUSIQUE_FILE_PATH)
    print("\n")
    print("="*80)
    print("bamboogle测试结果")
    analyze_trajectory_errors(BAMBOOGLE_FILE_PATH)