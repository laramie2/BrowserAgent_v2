...
def load_and_fill_success_fields(file_path):
    ...
    if (
        ...
        fd, tmp_path = tempfile.mkstemp(prefix=".analyze_failures_", suffix=".jsonl", dir=output_dir, text=True)
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyze BrowserAgent trajectory errors")
    parser.add_argument("result_dir", help="directory containing <benchmark>_test_results.jsonl")
    parser.add_argument("--benchmarks", nargs="+", default=["nq", "triviaqa", "popqa", "hotpot", "2wiki", "musique", "bamboogle"])
    args = parser.parse_args()
    for benchmark in args.benchmarks:
        result_file = os.path.join(args.result_dir, f"{benchmark}_test_results.jsonl")
        if not os.path.isfile(result_file):
            print(f"warning: missing {result_file}")
            continue
        print("=" * 80)
        print(f"{benchmark} results")
        analyze_trajectory_errors(result_file)
