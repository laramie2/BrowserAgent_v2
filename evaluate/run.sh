...
SCRIPT_PATH="${EVAL_DIR}/$(basename "${BASH_SOURCE[0]}")"
LOG_DIR="${EVAL_DIR}/logs"
RUN_LOG="${LOG_DIR}/eval_runner_${RUN_ID}.log"

DEFAULT_MODEL_PATH="${ROOT_DIR}/RL/models/browseragent-sft"
DEFAULT_OUTPUT_DIR="${EVAL_DIR}/results/$(basename "${DEFAULT_MODEL_PATH}")"
DEFAULT_PROMPT_PATH="${ROOT_DIR}/prompt/system_prompt_with_history_info.txt"

# 单跳数据集
DEFAULT_NQ_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/nq/test-00000-of-00001.parquet"
DEFAULT_TRIVIAQA_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/triviaqa/test-00000-of-00001.parquet"
DEFAULT_POPQA_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/popqa/test-00000-of-00001.parquet"

# 多跳数据集
DEFAULT_HOTPOT_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/hotpot/validation-00000-of-00001.parquet"
DEFAULT_2WIKI_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/2wiki/validation-00000-of-00001.parquet"
DEFAULT_MUSIQUE_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/musique/validation-00000-of-00001.parquet"
DEFAULT_BAMBOOGLE_DATA_PATH="${ROOT_DIR}/RL/dataset/BrowserAgent-SeedData/bamboogle/test-00000-of-00001.parquet"
...
usage() {
    cat <<'EOF'
Usage:
  ./evaluate/run.sh [nq|popqa|hotpot|2wiki|musique|bamboogle|triviaqa|all ...] [options]
  ./evaluate/run.sh --benchmarks hotpot,nq,popqa --model-path /path/to/model --num-workers 32
...
Common options:
  -b, --benchmarks LIST           Benchmarks: nq,popqa,hotpot,2wiki,musique,bamboogle,triviaqa,all.
      ...
      --pipeline-python PATH      Python executable for evaluate.pipeline.
      ...
      --no-use-vlm                Do not pass --use_vlm to evaluate.pipeline.
...
Environment variables with the same uppercase names can also be used, for example:
  VLLM_MODEL_PATH=/path/to/model NUM_WORKERS=32 ./evaluate/run.sh all
EOF
}
...
run_benchmark_once() {
    ...
    cmd=(
        "${PIPELINE_PYTHON}" -m evaluate.pipeline
...
run_token_stats() {
    ...
    local cmd=(
        "${PIPELINE_PYTHON}" -m evaluate.token_stats
        --input "${OUTPUT_DIR}"
